...
class DashboardLimbahCairController extends Controller
{
    function dashboardLimbahCairData(Request $request)
    {
        ...
        $all_reports_year = MLimbahCair::where('statusactive_limbah_cair', 1)
            ->where('tahun', $tahun)
            ->get()
            ->groupBy('periode');
        ...
        // Check if current user has reported for this period using preloaded data
        $laporan_periode_now = $all_reports_year[$periode] ?? collect();
        $current_user_reported = $laporan_periode_now->contains('id_user', $form_id_user);
        $laporan['sudah_lapor'] = $current_user_reported;
        ...
        foreach ($all_users as $value) {
            ...
            $value->sudah_lapor = $value->sudah_lapor_limbah_cair;
        }
